"""Optimized Outlook client for faster mailbox search."""
...
class OutlookClientOptimized:
    """Optimized client for accessing Outlook mailboxes with better performance."""
    ...
    def _extract_email_data_optimized(self, item, folder_name: str, 
        ...
        """Extract only essential email data for better performance."""
        try:
            ...
            return {
                ...
                # Only get body preview, not full body for search results
                'body': getattr(item, 'Body', '')[:500] if config.get_int('max_body_chars', 0) != 0 else '',
...
outlook_client_optimized = OutlookClientOptimized()
